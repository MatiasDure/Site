...
**Markdown parsing**: `gray-matter` (frontmatter) + `remark` + `remark-rehype` + `rehype-sanitize` + `rehype-stringify` (body → sanitized HTML)
...
| 5 new packages added (`gray-matter`, `remark`, `remark-rehype`, `rehype-sanitize`, `rehype-stringify`), each justified below | Technology Stack | ✅ |
...
| `remark` + `remark-rehype` + `rehype-sanitize` + `rehype-stringify` | Convert Markdown body to sanitized HTML for detail page | `marked` is less maintained; `remark-html` has no explicit sanitization step; rehype pipeline provides explicit XSS protection without jsdom overhead
...
│
├── lib/
│   ├── projects.ts              ← getAllProjects(domain?), getProject(domain, slug), getFeaturedProjects(domain); uses rehype-sanitize for XSS protection
...
- [X] P0-1 Install packages: `gray-matter`, `remark`, `remark-rehype`, `rehype-sanitize`, `rehype-stringify`
- [X] P0-2 Create `app/types/project.ts` — `Domain` union type (`'web' | 'app' | 'game' | 'embedded'`), `Project` interface (slug, title, description, coverImage, tags, featured, demo?, repo?, htmlBody)
- [X] P0-3 Create `app/types/activity.ts` — `SpotifyTrack` (name, artist, albumArt) and `AnimeEntry` (title, url?) interfaces
- [X] P0-4 Create `app/types/index.ts` barrel
- [X] P0-5 Create `app/lib/projects.ts` — `getAllProjects(domain?: Domain): Promise<Project[]>` reads all `.md` files from `projects/<domain>/`, parses frontmatter with gray-matter, converts body to sanitized HTML with `remark` → `remark-rehype` → `rehype-sanitize` → `rehype-stringify`; `getProject(domain, slug)` returns single project with `htmlBody`; `getFeaturedProjects(domain)` filters `featured: true` capped at 3
- [X] P0-6 Create `app/lib/spotify.ts` — `getRecentlyPlayed(): Promise<SpotifyTrack | null>` — POST to Spotify token endpoint with `SPOTIFY_REFRESH_TOKEN`, then GET `/v1/me/player/recently-played?limit=1`; returns null on any error
- [X] P0-7 Create `app/lib/myanimelist.ts` — `getFavoriteAnime(): Promise<AnimeEntry[]>` — GET `https://api.jikan.moe/v4/users/{MAL_USERNAME}/favorites`; returns empty array on error
- [X] P0-8 Create `.env.example` with `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, `SPOTIFY_REFRESH_TOKEN`, `MAL_USERNAME`
- [X] P0-9 Create `app/lib/index.ts` barrel
- [X] P0-10 Seed at least 1 sample `.md` file per domain under `projects/` so pages can render during development
...
- [X] P1-1 Create `app/components/NavBar.tsx` — site name/logo + links to each domain's all-projects page (`/projects/web`, `/projects/app`, etc.)
- [X] P1-2 Create `app/components/Footer.tsx` — copyright text + `<SocialLinks />`
- [X] P1-3 Create `app/components/SocialLinks.tsx` — LinkedIn and GitHub `<a>` links with accessible labels
- [X] P1-4 Create `app/components/HeroSection.tsx` — name, tagline, bio; uses `<SocialLinks />`
- [X] P1-5 Create `app/components/ProjectCard.tsx` — cover image via `<Image>`, title, short description; entire card is a `<Link>` to the detail page
- [X] P1-6 Create `app/components/DomainSection.tsx` — receives `domain` and `featuredProjects`; renders section heading, 3 × `<ProjectCard />`, "View All" `<Link>`
- [X] P1-7 Rewrite `app/layout.tsx` — import `<NavBar />` and `<Footer />`, set page `<Metadata>` with site title and description
- [X] P1-8 Rewrite `app/page.tsx` — `export const revalidate = 600`; fetches featured projects for all 4 domains; renders `<HeroSection />` + 4 × `<DomainSection />`
- [X] P1-9 Create `app/components/index.ts` barrel
- [X] P1-10 Verify mobile responsiveness: grid collapses to 1 col on sm, 2 on md, 3 on lg
...
- [X] P2-1 Create `app/projects/[domain]/page.tsx` — `generateStaticParams` returns `[{domain:'web'},{domain:'app'},{domain:'game'},{domain:'embedded'}]`; fetches all projects for the domain; renders page heading and responsive card grid
- [X] P2-2 Add empty-state when domain has 0 projects
- [X] P2-3 Add `<Link>` back to landing page (`/`) in page heading area
...
- [X] P3-1 Create `app/projects/[domain]/[slug]/page.tsx` — `generateStaticParams` enumerates all (domain, slug) pairs from `getAllProjects()`; renders project name, `htmlBody` via `dangerouslySetInnerHTML` (safe: sanitized at read-time by `rehype-sanitize`), tags, demo/repo links, cover image
- [X] P3-2 Add `<Link>` back to the domain all-projects page
- [X] P3-3 Add `generateMetadata` to set `<title>` and `<meta description>` per project
...
- [X] P4-1 Create `app/components/SpotifyCard.tsx` — album art via `<Image>` with fixed dimensions, track name, artist
- [X] P4-2 Create `app/components/AnimeCard.tsx` — accept `entry: AnimeEntry`; render as `<a href={entry.url} target="_blank" rel="noopener noreferrer">` wrapping a card with cover `<Image>` from `entry.imageUrl` and anime title; include accessible `aria-label` combining title and "— view on MyAnimeList"
- [X] P4-3 Create `app/components/ActivitySection.tsx` — calls `getRecentlyPlayed()` and `getFavoriteAnime()` in RSC; renders `<SpotifyCard />` and a grid of up to 5 `<AnimeCard />` components, or fallback text when data is null/empty
- [X] P4-4 Add `<ActivitySection />` to `app/page.tsx`
- [X] P4-5 Verify fallback renders without errors when env vars are absent (local dev without credentials)
