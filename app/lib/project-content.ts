...
export async function getMarkdownProjects(domain?: Domain): Promise<Project[]> {
  ...
  const projectsPerDomain = await Promise.all(
    domains.map(async (currentDomain) => {
      const directory = path.join(PROJECTS_ROOT, currentDomain);
      let files: string[];
      ...
      return Promise.all(markdownFiles.map((fileName) => readProjectFile(currentDomain, fileName)));
    })
  );
...
export async function getMarkdownProject(domain: Domain, slug: string): Promise<Project> {
